...
from inference.test_utils_jra import Test
...
def main():
    ...
    if args.post_processing:
        np.save(
            os.path.join(results_dir, f"jra_cycle_{args.n_cycle}_bias_mean.npy"), bias
        )
        np.save(os.path.join(results_dir, f"jra_cycle_{args.n_cycle}_fs_mean.npy"), fs)
        np.save(
            os.path.join(results_dir, f"jra_cycle_{args.n_cycle}_fs_mean_pers.npy"),
            fs_pers,
        )
        if args.ocean:
            np.save(
