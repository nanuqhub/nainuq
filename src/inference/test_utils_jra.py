...
class Test:
    def __init__(
        ...
        if "sic" in self.sea_ice_variables:
            self.cmin = 0.0005 - self.mean_input[1] / self.std_input[1]
    ...
    def normalize_input(self, x):
        ...
        if "sit" in self.sea_ice_variables:
            ...
            self.cmin = 5e-4 - self.mean_input[1] / self.std_input[1]
            self.c0 = -self.mean_input[1] / self.std_input[1]
        ...
        for t in range(1, self.k):
            ...
            if self.ocean:
                ...
                x_inputs[t] = np.concatenate(
                    (
                        x_pred_SI[t],  # Current prediction
                        x_truth[t, self.N : self.N_inputs, :, :],
                    ),  # Future forcings
                    axis=0,
                )
    ...
    def test_model(self):
        ...
        N_file = len(list(Path(f"{self.path_data}test/").rglob("*jra*.tfrecords.*")))

        # Load files
        # val_files = [f"{self.path_data}test/data_2018_jra.tfrecords.{str(i).zfill(3)}" for i in range(N_file)]
        ...
        def load_year(year):
            N_files = len(
                list(
                    Path(f"{self.path_data}test/").rglob(f"data_{year}_jra.tfrecords.*")
                )
            )
            files = [
                f"{self.path_data}test/data_{year}_jra.tfrecords.{str(i).zfill(3)}"
        ...
        total_steps_needed = (
            self.frequency // self.timestep * (self.N_cycle - 1) + self.k + 1
        )
        ...
        for i in trange(self.N_cycle):
            if self.ocean:
                (
                    ...
                    xpred_ocean,
                    xtruth_ocean,
                ) = self.apply_emulator(x[i : i + self.k * self.timestep + 1])
                ...
                (
                    ...
                    x[
                        i * self.frequency : i * self.frequency
                        + self.k * self.timestep
                        + 1
                    ]
                )
                bias_SI[i] = bias_res_SI.reshape((self.k, self.N))
            ...
            if self.save_pred:
                ...
                np.save(
                    f"{self.path}test_jra_{self.post_processing}_cycle_{self.N_cycle}_truth_{i}.npy",
                    tru_SI,
                )
                np.save(
                    f"{self.path}test_jra_{self.post_processing}_cycle_{self.N_cycle}_pred_{i}.npy",
                    xpred_SI,
                )
                if self.ocean:
                    np.save(
                        f"{self.path}post_process_{self.post_processing}_cycle_{self.N_cycle}_truth_ocean{i}.npy",
                        tru_ocean,
                    )
                    np.save(
                        f"{self.path}post_process_{self.post_processing}_cycle_{self.N_cycle}_pred_ocean{i}.npy",
                        xpred_ocean,
                    )
        ...
        if self.N_cycle > 100:
            ...
            np.save(
                f"{self.path}field_{self.post_processing}_cycle_{self.N_cycle}_pred_24h.npy",
                x_pred1,
            )

            x_pred10 = x_pred_SI[:, N_freq * 10 : N_freq * 10 + N_freq].mean(axis=1)
            np.save(
                f"{self.path}field_{self.post_processing}_cycle_{self.N_cycle}_pred_10d.npy",
                x_pred10,
            )
        if self.ocean:
            return (
